...
import { Package, AlertTriangle, TrendingDown, Warehouse, Search, SlidersHorizontal } from "lucide-react";
import { mockProducts, mockStockAlerts } from "@/lib/mock-data";
...
const stockData = [
  { category: "Elektronik", stock: 430 }, { category: "Aksesori", stock: 280 },
  { category: "Storage", stock: 195 }, { category: "Fashion", stock: 310 },
  { category: "Lainnya", stock: 133 },
];
...
export default function InventoryPage() {
  ...
  const lowStockCount = mockProducts.filter((p) => p.is_low_stock).length;
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Produk" value="1,248" change={2.1} icon={Package} iconBg="bg-indigo-500/15" iconColor="text-indigo-400" />
          <StatCard title="Stok Rendah" value={String(lowStockCount)} icon={AlertTriangle} iconBg="bg-amber-500/15" iconColor="text-amber-400" />
          <StatCard title="Produk Habis" value="4" icon={TrendingDown} iconBg="bg-red-500/15" iconColor="text-red-400" />
          <StatCard title="Gudang Aktif" value="3" icon={Warehouse} iconBg="bg-cyan-500/15" iconColor="text-cyan-400" />
        </div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stockData} barSize={36}>
                ...
                <Bar dataKey="stock" name="Stok" radius={[6, 6, 0, 0]}>
                  {stockData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              {mockStockAlerts.map((item) => (
                <div key={item.id} className="p-3 rounded-lg bg-white/[0.03] border border-white/[0.04]">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full border font-semibold ${getStatusColor(item.status)}`}>
                      {item.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${item.status === "critical" ? "bg-red-500" : "bg-amber-500"}`}
                        style={{ width: `${Math.min((item.stock / item.minimum) * 100, 100)}%` }}
                      />
                    </div>
                    <span className="text-[10px] text-white/40 shrink-0">{item.stock}/{item.minimum}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] overflow-hidden">
          <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
            <div>
              ...
              <p className="text-xs text-white/30 mt-0.5">{filtered.length} produk ditemukan</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <tbody>
                {filtered.map((product, i) => (
                  <tr key={product.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs text-white/40">{product.supplier}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-semibold ${product.stock <= 10 ? "text-red-400" : product.stock <= 20 ? "text-amber-400" : "text-emerald-400"}`}>
                          {product.stock}
                        </span>
                        <div className="w-12 h-1 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${product.stock <= 10 ? "bg-red-500" : product.stock <= 20 ? "bg-amber-500" : "bg-emerald-500"}`}
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs font-semibold text-white/70">{formatCurrency(product.price)}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`text-[10px] px-2 py-1 rounded-full border font-medium ${product.is_low_stock ? getStatusColor("low") : getStatusColor("active")}`}>
                        {product.is_low_stock ? "Stok Rendah" : "Normal"}
                      </span>
                    </td>
                  </tr>
                ))}
